    ...
    if os.path.exists(path) and os.path.isdir(path):
        for prj in os.listdir(path):
            ...
            if code != 0:
                ...
                code, _ = write(_config_path, config_data, is_json=True)
            if code == 0:
                config_data["name"] = prj
                config_data["atime"] = (
                    time.strftime(
                        "%d/%m/%Y %I:%M:%S %p",
                        time.localtime(os.path.getatime(_config_path)),
                    ),
                )  # 访问时间
                config_data["mtime"] = (
                    time.strftime(
                        "%d/%m/%Y %I:%M:%S %p",
                        time.localtime(os.path.getmtime(_config_path)),
                    ),
                )  # 修改时间
                config_data["ctime"] = (
                    time.strftime(
                        "%d/%m/%Y %I:%M:%S %p",
                        time.localtime(os.path.getctime(_config_path)),
                    ),
                )  # 创建时间

                config_data["atime"] = (
                    config_data["atime"][0]
                    if isinstance(config_data["atime"], tuple)
                    else config_data["atime"]
                )
                config_data["mtime"] = (
                    config_data["mtime"][0]
                    if isinstance(config_data["mtime"], tuple)
                    else config_data["mtime"]
                )
                config_data["ctime"] = (
                    config_data["ctime"][0]
                    if isinstance(config_data["ctime"], tuple)
                    else config_data["ctime"]
                )
...
def get_project(path):
    if os.path.exists(path):
        ...
        config_data["atime"] = (
            time.strftime(
                "%d/%m/%Y %I:%M:%S %p", time.localtime(os.path.getatime(_config_path))
            ),
        )  # 访问时间
        config_data["mtime"] = (
            time.strftime(
                "%d/%m/%Y %I:%M:%S %p", time.localtime(os.path.getmtime(_config_path))
            ),
        )  # 修改时间
        config_data["ctime"] = (
            time.strftime(
                "%d/%m/%Y %I:%M:%S %p", time.localtime(os.path.getctime(_config_path))
            ),
        )  # 创建时间
